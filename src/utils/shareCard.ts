...
import { resolveFeedManifest, stainsForPage, type PaperManifest } from './paperSoul';
import type { ReadingFeedItem } from './readingFeed';

const SHARE_WIDTH = 1080;
const SHARE_HEIGHT = 1350;
const PAPER_BG = '#f4ead5';
const PAPER_INK = '#4a3f35';
...
export async function renderFragmentShareCard(input: FragmentShareCardInput): Promise<Blob> {
  ...
  const manifest = resolveFeedManifest(
    input.manifest,
    input.item.filename || input.item.bookId || input.item.id
  );
  context.fillStyle = manifest.paletteTint || PAPER_BG;
  context.fillRect(0, 0, SHARE_WIDTH, SHARE_HEIGHT);

  const active = stainsForPage(manifest, input.paperPage || 1);
  ...
  const wash = context.createRadialGradient(
    SHARE_WIDTH * 0.5, SHARE_HEIGHT * 0.45, SHARE_HEIGHT * 0.2,
    SHARE_WIDTH * 0.5, SHARE_HEIGHT * 0.5, SHARE_HEIGHT * 0.75
  );
  wash.addColorStop(0, 'rgba(90, 60, 30, 0)');
  wash.addColorStop(1, 'rgba(90, 60, 30, 0.14)');
