...
function sourceSignature(books) {
  return books
    .map((book) => {
      ...
      // Git/Vercel checkout mtimes are not stable. Keep the committed feed
      // reusable across builds while still invalidating on size changes.
      return `${book.filename}:${stat.size}`;
    })
    .sort()
    .join('|');
}
